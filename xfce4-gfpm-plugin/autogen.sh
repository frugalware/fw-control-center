...
gen_pot()
{
	...
	intltool-update --pot --gettext-package=xfce4-gfpm-plugin
	rm intltool-extract
...
import_pootle()
{
	...
	if [ -d $po_dir ]; then
		...
		do
			[ -e $po_dir/fwcontrolcenter/$i/xfce4-gfpm-plugin.po ] || continue
			cp $po_dir/fwcontrolcenter/$i/xfce4-gfpm-plugin.po po/$i.po
...
if [ "$1" == "--dist" ]; then
	...
	cd ..
	tar czf xfce4-gfpm-plugin-$ver.tar.gz xfce4-gfpm-plugin-$ver
	rm -rf xfce4-gfpm-plugin-$ver
	gpg --comment "See http://ftp.frugalware.org/pub/README.GPG for info" \
		-ba -u 20F55619 xfce4-gfpm-plugin-$ver.tar.gz
	exit 0
fi
